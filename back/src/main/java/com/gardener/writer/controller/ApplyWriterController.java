...
import com.gardener.writer.service.WriterApplyService;
import com.google.gson.Gson;

@WebServlet("/apply")
public class ApplyWriterController extends HttpServlet {
	private WriterApplyService service;

	public ApplyWriterController() {
		service = WriterApplyService.getInstance();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (memberId != 0) {
            boolean writerInserted = service.insertWriter(memberId);
            if (writerInserted) {
